...
for col in tqdm(df.columns):
    if col not in ['customer_ID','S_2']:
        ...
        bins = GreedyFindBin(vc.index.values,vc.values,len(vc),31,vc.sum())
        df[col] = np.digitize(df[col],[-np.inf]+bins)
        df.loc[df[col]==len(bins)+1,col] = 0
        df[col] = df[col] / df[col].max()

df.to_feather(f'{input_folder}nn_all_feature.feather')

df = pd.read_feather(f'{input_folder}train.feather')
...
df.to_feather(f'{input_folder}df_nn_feature_train.feather')

df = pd.read_feather(f'{input_folder}test.feather')
...
df.to_feather(f'{input_folder}df_nn_feature_test.feather')


print("S2 feature engineering done!")
