...
parser.add_argument("--lr", type=float, default=10000e-7)
...
def Lgb_train(train, config, gkf=False, aug=None, output_root='./output/', run_id=None):
    if not run_id:
        run_id = 'run_lgb_' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        while os.path.exists(output_root+run_id+'/'):
            time.sleep(1)
...
def Lgb_predict(test, config, gkf=False, aug=None, output_root='./output/', run_id=None):
    if not run_id:
        run_id = 'run_lgb_' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        while os.path.exists(output_root+run_id+'/'):
            time.sleep(1)
...
def NN_train_and_predict(train, test, model_class, config, use_series_oof, logit=False, output_root='./output/', run_id=None):
    ...
    if train is not None:
        ...
        for fold, (trn_index, val_index) in enumerate(skf.split(train_y,train_y[label_name])):
            ...
            model = model_class(nn_series_sample_col_count,nn_feature_sample_col_count*2,1,3,hidden_dim,use_series_oof=use_series_oof)
            scheduler = Adam12()
...
def NN_train(train, model_class, config, use_series_oof, logit=False, output_root='./output/', run_id=None):
    if not run_id:
        run_id = 'run_nn_' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        while os.path.exists(output_root+run_id+'/'):
            time.sleep(1)
    ...
    if train is not None:
        ...
        for fold, (trn_index, val_index) in enumerate(skf.split(train_y,train_y[label_name])):
            ...
            # optimizer = torch.optim.Adam(model.parameters(), lr=10000e-7, weight_decay=1e-8)
            optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-8)
            
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=epochs)
            # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=0.8, last_epoch=-1, verbose='deprecated')
            ...
            if args.do_train:
                for epoch in range(epochs):
                    ...
                    for data in bar:
                        ...
                        # train_num += data['batch_feature'].shape[0]
                        # train_loss += data['batch_feature'].shape[0] * loss.item()
                        train_num += data['batch_y'].shape[0]
                        train_loss += data['batch_y'].shape[0] * loss.item()
                        bar.set_description('loss: %.4f' % (loss.item()))
        ...
        if True:
            ...
            formatted_time = current_time.strftime('__%Y_%m_%d_%H_%M_%S__')
            os.rename(output_path + 'train.log', output_path + 'train' + formatted_time + '_%.6f.log'%mean_valid_metric)
...
def NN_predict(test, model_class, config, use_series_oof, logit=False, output_root='./output/', run_id=None):
    # df_sample = pd.read_feather(f'./input/all_feature_sample.feather')
    # num_column_count = len(df_sample.columns)-13-1
    # del df_sample
